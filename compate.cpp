...
// tree definition online from canvas I think
struct TreeNode {
    int data;
    TreeNode *left;
    TreeNode *right;
...
Pair max_sum(TreeNode*root){
    ...
    if(left.isbst and right.isbst){
        if(left.maxLeft<root->data and root->data<right.minRight){
            p.sum+=left.sum+right.sum+root->data;
            ans=max(p.sum,ans);
            p.maxLeft=max(root->data,right.maxLeft);
            p.minRight=min(root->data,left.minRight);
            p.isbst=true;
            return p;
        }
    }
