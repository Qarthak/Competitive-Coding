...
int main(){
	...
	while(t--){
		...
		ll sumC=0;
		ll sumV=0;
		sumV=hash[0]+hash['e'-'a']+hash['i'-'a']+hash['o'-'a']+hash['u'-'a'];
		maxV=max({hash[0],hash['e'-'a'],hash['i'-'a'],hash['o'-'a'],hash['u'-'a']});
		for(int i=0; i<26;i++){
			if(i!=0 && i!='e'-'a' && i!='i'-'a' && i!='o'-'a' && i!='u'-'a')
				maxC=max(hash[i]*1LL,maxC);
		}
		ll tot=accumulate(hash,hash+26,0);
		sumC=tot-sumV;
		cout << min(2*(sumV-maxV)+sumC,2*(sumC-maxC)+sumV) << endl ;
	}
}